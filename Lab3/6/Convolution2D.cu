...
__constant__ double filter[MAX_FILTER_SIZE];
...
int main(int argc, char **argv) {
	...
	*h_OutputCPU = NULL, *d_OutputGPU = NULL, *h_OutputGPU =  NULL;
	...
	err = cudaMalloc((void**) &d_Input, imageW * imageH * sizeof(double));
	CHECK_CUDA_ERROR(err, 1, d_Input)

	err = cudaMalloc((void**) &d_Buffer, imageW * imageH * sizeof(double));
	CHECK_CUDA_ERROR(err, 2, d_Buffer, d_Input)

	err = cudaMalloc((void**) &d_OutputGPU, imageW * imageH * sizeof(double));
	...
	cudaMemcpyToSymbol(filter, h_Filter, FILTER_LENGTH*sizeof(double));
