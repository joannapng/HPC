...
#define NEW_WIDTH (imageW + 2 * filter_radius)
#define NEW_HEIGHT (imageH + 2 * filter_radius)
#define PAD filter_radius
// #define accuracy .5
float accuracy;
	...
	for (y = filterR; y < imageH - filterR; y++) {
		for (x = filterR; x < imageW - filterR; x++) {
			...
			for (k = -filterR; k <= filterR; k++) {
				...
				h_Dst[(y - filterR)* (imageW - 2 * filterR) + x - filterR] = sum;
			}
		}
	}
...
__global__ void convolutionRowGPU(float *d_Dst, float *d_Src, 
	...
	for (k = -filterR; k <= filterR; k++) {
		int d = idx + k;
		sum += d_Src[(idy + filterR ) * imageW + d + filterR] * filter[filterR - k];     
		
	}
	d_Dst[ ( idy + filterR ) * imageW  + idx + filterR] = sum;
}
...
__global__ void convolutionColumnGPU(float *d_Dst, float *d_Src,
	...
	for (k = -filterR; k <= filterR; k++) {
		int d = idy + k;
		sum += d_Src[(d + filterR) * imageW + idx + filterR] * filter[filterR - k];   
	}
  	d_Dst[ idy* (imageW - 2* filterR) + idx] = sum;
}
...
int main(int argc, char **argv) {
	...
	*h_OutputCPU = NULL, *d_OutputGPU = NULL, *h_OutputGPU =  NULL;
	...
	// CUDA events for time measurement
	...
	h_Input     = (float *)calloc(NEW_WIDTH * NEW_HEIGHT, sizeof(float));
	h_Buffer    = (float *)calloc(NEW_WIDTH * NEW_HEIGHT, sizeof(float));
	...
	for (int x=PAD; x < NEW_WIDTH-PAD; x++) {
		for (int y=PAD; y<NEW_HEIGHT-PAD; y++) {
			h_Input[y * NEW_WIDTH + x] = (float)rand() / ((float)RAND_MAX / 255) + (float)rand() / (float)RAND_MAX;
		}
	}
	...
	convolutionRowCPU(h_Buffer, h_Input, h_Filter, NEW_WIDTH, NEW_HEIGHT, filter_radius); // convolution kata grammes
	gettimeofday(&CPU_row, NULL);
	convolutionColumnCPU(h_OutputCPU, h_Buffer, h_Filter, NEW_WIDTH, NEW_HEIGHT, filter_radius); // convolution kata sthles
	...
	err = cudaMalloc((void**) &d_Input, NEW_WIDTH * NEW_HEIGHT * sizeof(float));
	CHECK_CUDA_ERROR(err, 1, d_Input)


	err = cudaMalloc((void**) &d_Buffer, NEW_WIDTH * NEW_HEIGHT * sizeof(float));
	CHECK_CUDA_ERROR(err, 2, d_Buffer, d_Input)

	err = cudaMemset(d_Buffer, 0, NEW_WIDTH*NEW_HEIGHT * sizeof(float));
	CHECK_CUDA_ERROR(err, 2, d_Buffer, d_Input)

	err = cudaMalloc((void**) &d_OutputGPU, imageH * imageW * sizeof(float));
	...
	cudaMemcpy(d_Input, h_Input, NEW_HEIGHT*NEW_WIDTH*sizeof(float), cudaMemcpyHostToDevice);
	cudaMemcpyToSymbol(filter, h_Filter, FILTER_LENGTH*sizeof(float));	
	cudaEventRecord(GPU_input_transfers_stop);

	convolutionRowGPU<<<gridDim, blockDim>>>(d_Buffer, d_Input, NEW_WIDTH, NEW_HEIGHT, filter_radius);
	...
	convolutionColumnGPU<<<gridDim, blockDim>>>(d_OutputGPU, d_Buffer, NEW_WIDTH, NEW_HEIGHT, filter_radius);
	err = cudaGetLastError();
	CHECK_CUDA_ERROR(err, 3, d_Buffer, d_Input, d_OutputGPU)
