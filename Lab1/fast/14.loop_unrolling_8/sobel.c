...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) {
		...
		for (j=1; j<SIZE-7; j+=8 ) {
			...
			p5 = convolution2D_hor(i, j + 4, input) + 
				convolution2D_ver(i, j + 4, input);
			/* If the resulting value is greater than 255, clip it *
			 * to 255.											   */
			if (p5 >= 65536)
				output[idx + j + 4] = 255;      
			else
				output[idx + j + 4] = (unsigned char)(int)sqrt(p5);

			p6 = convolution2D_hor(i, j+5, input) + 
				convolution2D_ver(i, j+5, input);
			/* If the resulting value is greater than 255, clip it *
			 * to 255.											   */
			if (p6 >= 65536)
				output[idx + j+5] = 255;      
			else
				output[idx + j+5] = (unsigned char)(int)sqrt(p6);

			p7 = convolution2D_hor(i, j + 6, input) + 
				convolution2D_ver(i, j + 6, input);
			/* If the resulting value is greater than 255, clip it *
			 * to 255.											   */
			if (p7 >= 65536)
				output[idx + j + 6] = 255;      
			else
				output[idx + j + 6] = (unsigned char)(int)sqrt(p7);
			
			p8 = convolution2D_hor(i, j + 7, input) + 
				convolution2D_ver(i, j + 7, input);
			/* If the resulting value is greater than 255, clip it *
			 * to 255.											   */
			if (p8 >= 65536)
				output[idx + j + 7] = 255;      
			else
				output[idx + j + 7] = (unsigned char)(int)sqrt(p8);
