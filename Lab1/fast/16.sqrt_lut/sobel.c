 ...
 * pixel we process.														  */
inline int convolution2D_ver(int posy, int posx, const unsigned char *restrict input) {
	register int res;
...
inline int convolution2D_hor(int posy, int posx, const unsigned char *restrict input) {
	register int res;
	...
	for (i=1; i<SIZE-1; i+=1) {
		...
		for (j=1; j<SIZE-1; j+=1 ) {
			...
			if (p >= 65536)
				...
				output[idx + j] = (unsigned char)(int)sqrt_lookup[p];
		}
	}
