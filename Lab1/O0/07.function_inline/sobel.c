...
/* Implement a 2D convolution of the matrix with the vertical operator */
...
inline int convolution2D_ver(int posy, int posx, const unsigned char *input) {
	...
	/* The center column of the vertical operator is zero, so we omit those multiplications */
	/* For the elements of the operator that are -1 or 1, we subtract or add the input element  *
	 * respectively. For the elements that are -2 or 2, we shift the input by 2 and subtract	* 
	 * or add it respectively.																	*/
	
	/* column #1 */
	res += input[(posy - 1) * SIZE + posx - 1 ];
	res += input[(posy - 1) * SIZE + posx ] << 1;
	res += input[(posy - 1) * SIZE + posx + 1 ];
...
/* Implement a 2D convolution of the matrix with the horizontal operator */
/* posy and posx correspond to the vertical and horizontal disposition of the *
 * pixel we process in the original image, input is the input image and       *
 * operator the operator we apply (horizontal or vertical). The function ret. *
 * value is the convolution of the operator with the neighboring pixels of the*
...
inline int convolution2D_hor(int posy, int posx, const unsigned char *input) {
	...
	/* The center row of the horizontal operator is zero, so we omit those multiplications */
	/* For the elements of the operator that are -1 or 1, we subtract or add the input element  *
	 * respectively. For the elements that are -2 or 2, we shift the input by 2 and subtract	* 
	 * or add it respectively.																	*/
	...
	for (i=1; i<SIZE-1; i+=1) {
		for (j=1; j<SIZE-1; j+=1 ) {
			...
			*/
			...
			// p = pow(convolution2D_hor(i, j, input), 2) + 
			// 	pow(convolution2D_ver(i, j, input), 2);
			p = pow(convolution2D_hor(i, j, input), 2) + pow(convolution2D_ver(i, j, input), 2);
			res = (int)sqrt(p);
