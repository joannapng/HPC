...
int convolution2D_ver(int index, const unsigned char *input);
int convolution2D_hor(int index, const unsigned char *input);
 ...
 * pixel we process.														  */
inline int convolution2D_ver(int index, const unsigned char *restrict input) {
	register int res;
  
	res = 0;

	
	res += input[index - SIZE- 1];
	res += input[index - SIZE] << 1;
	res += input[index - SIZE + 1];
...
inline int convolution2D_hor(int index, const unsigned char *input) {
	register int res;
...
{
	register double PSNR = 0;
	double t;
	register int i, j, idx;
	register unsigned int p1, p2, p3, p4, p5, p6, p7, p8;
	...
	for (i=SIZE+1; i<=(SIZE-1) * SIZE - 6 ; i+=8) {
			/* Apply the sobel filter and calculate the magnitude *
			 * of the derivative.								  */
		p1 = convolution2D_hor(i, input) + 
			convolution2D_ver(i, input);
		/* If the resulting value is greater than 255, clip it *
			* to 255.											   */
		if (p1 >= 65536)
			output[i] = 255;      
		else
			output[i] = (unsigned char)(int)sqrt(p1);

		p2 = convolution2D_hor(i + 1, input) + 
			convolution2D_ver(i + 1, input);
		/* If the resulting value is greater than 255, clip it *
			* to 255.											   */
		if (p2 >= 65536)
			output[i + 1] = 255;      
		else
			output[i + 1] = (unsigned char)(int)sqrt(p2);

		p3 = convolution2D_hor(i + 2, input) + 
			convolution2D_ver(i + 2, input);
		/* If the resulting value is greater than 255, clip it *
			* to 255.											   */
		if (p3 >= 65536)
			output[i + 2] = 255;      
		else
			output[i + 2] = (unsigned char)(int)sqrt(p3);

		p4 = convolution2D_hor(i + 3, input) + 
			convolution2D_ver(i + 3, input);
		/* If the resulting value is greater than 255, clip it *
			* to 255.											   */
		if (p4 >= 65536)
			output[i + 3] = 255;      
		else
			output[i + 3] = (unsigned char)(int)sqrt(p4);

		p5 = convolution2D_hor(i + 4, input) + 
			convolution2D_ver(i + 4, input);
		/* If the resulting value is greater than 255, clip it *
			* to 255.											   */
		if (p5 >= 65536)
			output[i + 4] = 255;      
		else
			output[i + 4] = (unsigned char)(int)sqrt(p5);

		p6 = convolution2D_hor(i + 5, input) + 
			convolution2D_ver(i + 5, input);
		/* If the resulting value is greater than 255, clip it *
			* to 255.											   */
		if (p6 >= 65536)
			output[i + 5] = 255;      
		else
			output[i + 5] = (unsigned char)(int)sqrt(p6);

		p7 = convolution2D_hor(i + 6, input) + 
			convolution2D_ver(i + 6, input);
		/* If the resulting value is greater than 255, clip it *
			* to 255.											   */
		if (p7 >= 65536)
			output[i + 6] = 255;      
		else
			output[i + 6] = (unsigned char)(int)sqrt(p7);

		p8 = convolution2D_hor(i + 7, input) + 
			convolution2D_ver(i + 7, input);
		/* If the resulting value is greater than 255, clip it *
			* to 255.											   */
		if (p8 >= 65536)
			output[i + 7] = 255;      
		else
			output[i + 7] = (unsigned char)(int)sqrt(p8);
	}
