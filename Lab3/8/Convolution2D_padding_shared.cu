...
#define FREE_CUDA_MEMORY  cudaFree(d_Buffer);\
                          cudaFree(d_Input); \
...
__global__ void convolutionRowGPU(float *d_Dst, float *d_Src, 
	...
	float *Src_s = &shared[0];

	int x = idx - filterR;

	Src_s[threadIdx.x + threadIdx.y * (BLOCK_DIM_X + 2 * filterR)] = (x < 0) ? 0 : d_Src[id - filterR];

	x = idx + filterR;

	Src_s[threadIdx.x + 2*filterR + threadIdx.y * (BLOCK_DIM_X + 2 * filterR)] = (x > imageW - 1) ? 0 : \
	d_Src[id + filterR];
	...
	for (k = -filterR; k <= filterR; k++) {
		int d = threadIdx.x + k;
		sum += Src_s[threadIdx.y * (BLOCK_DIM_X + 2 * filterR) + d + filterR] * filter[filterR - k];
	}
...
__global__ void convolutionColumnGPU(float *d_Dst, float *d_Src,
	...
	Src_s[threadIdx.y * BLOCK_DIM_X + threadIdx.x] = (y < 0) ? 0 : \
		d_Src[id - filterR * imageW];

	y = idy + filterR;

	Src_s[(threadIdx.y + 2 * filterR) * BLOCK_DIM_X + threadIdx.x] = (y > imageH - 1) ? 0 :\
		d_Src[id + filterR * imageW];
	...
	for (k = -filterR; k <= filterR; k++) {
		int d = threadIdx.y + k;
		sum += Src_s[(d + filterR) * BLOCK_DIM_X + threadIdx.x] * filter[filterR - k];
	}
...
int main(int argc, char **argv) {
	...
	*h_OutputCPU = NULL, *d_OutputGPU = NULL, *h_OutputGPU =  NULL;
	...
	convolutionRowGPU<<<gridDim, blockDim, BLOCK_DIM_Y * (BLOCK_DIM_X + 2 * PAD) * sizeof(float)>>>(d_Buffer, d_Input, imageW, imageH, filter_radius);
	...
	//convolutionColumnGPU<<<gridDim, blockDim, NEW_HEIGHT * BLOCK_DIM_X * sizeof(float) >>>(d_OutputGPU, d_Buffer, imageW, imageH, filter_radius);
	convolutionColumnGPU<<<gridDim, blockDim, BLOCK_DIM_X * (BLOCK_DIM_Y + 2 * PAD) * sizeof(float)>>>(d_OutputGPU, d_Buffer, imageW, imageH, filter_radius);
	err = cudaGetLastError();
	CHECK_CUDA_ERROR(err, 3, d_Buffer, d_Input, d_OutputGPU)
	...
	for (int x=0; x <imageW; x++) {
		for (int y=0; y<imageH; y++) {
			if (ABS(h_OutputGPU[y*imageW+x] - h_OutputCPU[y*imageW+x]) > accuracy) {
				...
				printf("%f %f\n", h_OutputCPU[y*imageW + x], h_OutputGPU[y * imageW + x]);
				FREE_CUDA_MEMORY
				cudaDeviceReset();
				exit(EXIT_FAILURE);
			}
